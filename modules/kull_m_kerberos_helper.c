...
BOOL kull_m_kerberos_helper_net_getDC(LPCSTR fullDomainName, DWORD altFlags, LPSTR * fullDCName)
{
	BOOL status = FALSE;
	DWORD ret, size;
	PDOMAIN_CONTROLLER_INFOA cInfo = NULL;
	ret = DsGetDcNameA(NULL, fullDomainName, NULL, NULL, altFlags | DS_IS_DNS_NAME | DS_RETURN_DNS_NAME, &cInfo);
...
void kull_m_kerberos_helper_util_impersonateToGetData(PCSTR user, PCSTR domain, PCSTR password, PCSTR kdc, PSID *sid, DWORD *rid, DWORD *pNbDc, PDS_DOMAIN_CONTROLLER_INFO_1A *dcInfos)
{
	NTSTATUS status;
...
#pragma warning(pop)
	if(NT_SUCCESS(RtlAnsiStringToUnicodeString(&uUser, &aUser, TRUE)))
	{
		if(NT_SUCCESS(RtlAnsiStringToUnicodeString(&uKdc, &aKdc, TRUE)))
		{
			...
			{
				ret = DsMakePasswordCredentialsA(user, domain, password, &hRPCAuth);
				if(ret == ERROR_SUCCESS)
				{
					if(pNbDc && dcInfos)
					{
						ret = DsBindWithCredA(kdc, domain, hRPCAuth, &hPds);
						if(ret == ERROR_SUCCESS)
						{
							ret = DsGetDomainControllerInfoA(hPds, domain, 1, pNbDc, (PVOID *) dcInfos);
							if(ret == ERROR_SUCCESS)
